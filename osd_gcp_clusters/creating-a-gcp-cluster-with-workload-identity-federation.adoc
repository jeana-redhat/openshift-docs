...
[NOTE]
====
WIF supports the deployment of a private {product-title} on {{gcp-short}} cluster with Private Service Connect (PSC). Red Hat recommends using PSC when deploying private clusters.
...
* For information about {product-title} clusters using a Customer Cloud Subscription (CCS) model on {{gcp-short}}, see xref:../osd_planning/gcp-ccs.adoc#ccs-gcp-customer-requirements_gcp-ccs[Customer requirements].
