...
{product-title} {product-version} is based on {op-system-base-full} 9.2. {op-system-base} 9.2 has been submitted for FIPS validation. Red Hat expects, though cannot commit to a specific timeframe, to obtain FIPS validation for {op-system-base} 9.0 and {op-system-base} 9.2 modules, and later even minor releases of {op-system-base} 9.x. Updates are available in link:https://access.redhat.com/articles/2918071[Compliance Activities and Government Standards].
...
In {product-title} {product-version}, you can install {lvms} in disconnected environments. For more information, see xref:../storage/persistent_storage_local/persistent-storage-using-lvms.adoc#lvms-installing-lvms-disconnected-env_logical-volume-manager-storage[Installing LVM Storage in a disconnected environment].
...
{product-title} release 4.13.43, which includes security updates, is now available. The list of bug fixes that are included in the update is documented in the link:https://access.redhat.com/errata/RHSA-2024:3494[RHSA-2024:3494] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHSA-2024:3496[RHSA-2024:3496] advisory.
...
{product-title} release 4.13.44, which includes security updates, is now available. The list of bug fixes that are included in the update is documented in the link:https://access.redhat.com/errata/RHSA-2024:3885[RHSA-2024:3885] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2024:3887[RHBA-2024:3887] advisory.
...
+
With this update, the Ingress Operator no longer adds the `clientca-configmap` finalizer to an `IngressController` object that is marked for deletion. As a result, the Ingress Operator no longer tries to perform erroneous updates, and no longer logs the associated errors. (link:https://issues.redhat.com/browse/OCPBUGS-34765[OCPBUGS-34765])
...
{product-title} release 4.13.45, which includes security updates, is now available. The list of bug fixes that are included in the update is documented in the link:https://access.redhat.com/errata/RHSA-2024:4484[RHSA-2024:4484] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHSA-2024:4486[RHSA-2024:4486] advisory.
...
* Previously, the {azure-first} node controller container did not tolerate the `NoExecute` taint on nodes. As a result, a node was uninitialized. With this release, the node controller deployment receives an update to tolerate the `NoExecutre` taint, so that nodes can be properly initialized. (link:https://issues.redhat.com/browse/OCPBUGS-35562[OCPBUGS-35562])

* Previously, the `wait-for-ceo` command used during the bootstrapping process to verify etcd rollout did not report errors for some failure modes. With this release, those error messages are visible on the `bootkube` script if the command exits in an error case. (link:https://issues.redhat.com/browse/OCPBUGS-35077[OCPBUGS-35077])
...
{product-title} release 4.13.48, which includes security updates, is now available. The list of bug fixes that are included in the update is documented in the link:https://access.redhat.com/errata/RHSA-2024:5444[RHSA-2024:5444] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHSA-2024:5446[RHSA-2024:5446] advisory.
...
* Previously, for clusters upgraded from older versions of {product-title}, enabling `kdump` on an OVN-enabled cluster sometimes prevented the node from rejoining the cluster or returning to the `Ready` state. This fix removes problematic stale data from older {product-title} versions and ensures this kind of stale data is always cleaned up. The node can now start correctly and rejoin the cluster. (link:https://issues.redhat.com/browse/OCPBUGS-38295[OCPBUGS-38295])

* Previously, a machine config pool (MCP) with a higher `maxUnavailable` value than the actual amount of unavailable nodes caused cordoned nodes to receive updates if they were in a certain position on the node list. With this fix, cordoned nodes are not added to a queue to receive updates. (link:https://issues.redhat.com/browse/OCPBUGS-37783[OCPBUGS-37783])
...
{product-title} release 4.13.50, which includes security updates, is now available. The list of bug fixes that are included in the update is documented in the link:https://access.redhat.com/errata/RHSA-2024:6691[RHSA-2024:6691] advisory. There are no RPM packages in this release.
...
{product-title} release 4.13.51, which includes security updates, is now available. The list of bug fixes that are included in the update is documented in the link:https://access.redhat.com/errata/RHSA-2024:6811[RHSA-2024:6811] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2024:6813[RHBA-2024:6813] advisory.
...
/ 4.13.61
[id="ocp-4-13-61_{context}"]
=== RHSA-2025:18241 - {product-title} 4.13.61 bug fix and security updates

Issued: 18 September 2025

{product-title} release 4.13.61, which includes security updates, is now available. The list of bug fixes that are included in the update is documented in the link:https://access.redhat.com/errata/RHSA-2025:18241[RHSA-2025:18241] advisory. The RPM packages that are included in the update are provided by the link:https://access.redhat.com/errata/RHBA-2025:18215[RHBA-2025:18215] advisory.

You can view the container images in this release by running the following command:

[source,terminal]
----
$ oc adm release info 4.13.61 --pullspecs
----
...
//4.13.63
include::modules/zstream-4-13-63-about.adoc[leveloffset=+2]
