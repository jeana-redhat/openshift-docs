...
// Configuring an ACME issuer to solve DNS01 challenges by using explicit credentials on {gcp-short}
include::modules/cert-manager-acme-dns01-explicit-gcp.adoc[leveloffset=+1]

// Configuring an ACME issuer to solve DNS01 challenges by using ambient credentials on {gcp-short}
