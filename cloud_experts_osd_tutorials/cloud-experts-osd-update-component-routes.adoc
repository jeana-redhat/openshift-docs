...
//Article text
This guide demonstrates how to modify the hostname and TLS certificate of the Web console, OAuth server, and Downloads component routes in {product-title} on {{gcp-short}} version 4.14 and above.{fn-supported-versions}
...
* gcloud CLI (`gcloud`)
* An {product-title} on {{gcp-short}} cluster version 4.14 or higher
// +
// [NOTE]
// ====
