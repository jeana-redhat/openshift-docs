...
//{gcp-short} component secret permissions requirements (placeholder)
//include::modules/cco-short-term-creds-component-permissions-gcp.adoc[leveloffset=+2]

//OLM-managed Operator support for authentication with {gcp-short} Workload Identity
...
// Application support for {gcp-short} Workload Identity service account tokens
// Extra context so module can be reused within assembly (unset in module)
:context: gcp
// Attributes used in module with cloud-specific values (unset in module)
