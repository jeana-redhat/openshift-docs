...
{product-title} on {{gcp-short}} users can deploy a single machine pool in a specific availability zone that is part of a Multi-AZ cluster using the {cluster-manager} CLI (`ocm`). This option is especially useful in situations where a desired instance type is not available in all availability zones of a specific region as well as when your cluster does not need more than one of the desired instance types.
...
For more information about {{gcp-short}} instance types and availability zones, see the _Additional resources_ section.
