...
Mint mode is the default and recommended best practice setting for the CCO to use on the platforms for which it is supported. In this mode, the CCO uses the provided admin-level cloud credential to run the cluster.
