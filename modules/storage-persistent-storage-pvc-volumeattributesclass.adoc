...
Volume Attributes Classes in {product-title} is available only with AWS Elastic Block Storage (EBS) and {gcp-first} persistent disk (PD) Container Storage Interface (CSI).
...
* With {gcp-short} PD, volume modification using Volume Attributes Classes is only possible for hyperdisk-balanced disk types.
