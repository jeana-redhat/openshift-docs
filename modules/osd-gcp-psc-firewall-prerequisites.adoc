...
If you are using a firewall to control egress traffic from {product-title} on {{gcp-short}}, you must configure your firewall to grant access to certain domains and port combinations listed in the tables below. {product-title} requires this access to provide a fully managed OpenShift service.

[IMPORTANT]
====
Only {product-title} on {{gcp-short}} clusters deployed with Private Service Connect can use a firewall to control egress traffic.
====
...
. Add the following URLs for the {{gcp-short}} API endpoints to an allowlist:
+
[cols="6,1,6",options="header"]
|===
|Domain | Port | Function
