...
Besides the required customer procedures listed in _Required customer procedure_, there are other specific actions that you must take when creating an {product-title} cluster on {{gcp-short}} using Workload Identity Federation (WIF) as the authentication type.
